...
# Unlike most ALB metrics, UnHealthyHostCount is published whenever targets are
# registered — even with zero traffic. That makes it the most reliable alarm in
# this stack for a portfolio environment that sits idle.
...
# Error *rate*, not raw count: "10 errors" means very different things at 100 req/min
# and at 100k req/min.
#
# Two details that matter:
#   FILL(m2,0) — 5xx metrics are only published when non-zero, so without FILL the whole
#                expression goes missing (not zero) whenever there are no errors.
#   IF(m1 >= N) — low-volume guard. Without it, 1 error out of 2 requests reads as 50%.
resource "aws_cloudwatch_metric_alarm" "error_rate" {
  ...
  # Generated by the ALB itself: 502 (bad response from target), 503 (no healthy
  # targets), 504 (target timeout). An infrastructure-level signal.
  metric_query {
    id = "m2"
    metric {
  ...
  # Returned by the application itself. An application-level signal. Both are combined
  # here because from the user's side the distinction is invisible.
  metric_query {
    id = "m3"
    metric {
...
# p99, not Average — an average hides the tail, and the tail is what users feel.
# Threshold is deliberately generous: with Aurora Serverless v2 auto-pause, the first
# request after an idle period waits for the database to resume.
